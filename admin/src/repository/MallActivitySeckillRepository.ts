...
export class MallActivitySeckillRepository implements CrudRequest<MallActivitySeckill> {
    ...
    public createRequest = async (data: Partial<MallActivitySeckill>): Promise<any> => {
    ...
    public updateRequest = async (data: Partial<MallActivitySeckill>): Promise<any> => {
        data.updateTime = Date.now()
        data.updateBy = this.userStore._id
